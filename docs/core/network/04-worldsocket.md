...
- **Big-packet path** — `ServerPktHeader::isLargePacket()` (`ServerPktHeader.h:51`) returns true when payload size > `0x7FFF`. The 5-byte form prepends `0x80 | (size >> 16)` so the high bit signals "third length byte present" (`ServerPktHeader.h:36`). `ClientPktHeader::IsValidSize` enforces `size < 10240` for inbound (`WorldSocket.h:63`).
