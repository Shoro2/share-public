...
|---|----------|--------|--------|--------|---------|
| 1 | 900466 | Totems follow player | PlayerScript | not tested | DUMMY marker. Shared `custom_totem_follow_playerscript` also checks `HasAura(900466)`. |
| 2 | 900467 | Mana regen +2% per missing mana% | C++/PlayerScript | not tested | DUMMY marker. `custom_mana_regen_playerscript::OnPlayerUpdate` → every 5s: missingPct = (1 - curMana/maxMana) × 100 → EnergizeBySpell(maxMana × missingPct × 0.02 / 100). At 50% missing → +1% maxMana/5s. At 90% missing → +1.8% maxMana/5s. |
