...
| 3 | 901068 | Summons heal on death/despawn | C++/UnitScript | implemented | DUMMY marker. `custom_druid_summon_heal_unitscript::OnUnitDeath` → when the summon dies + Owner HasAura(901068) → CastSpell(901073 Nature Bloom). |
| 4 | 901069 | Thorns → chance to cast Rejuv | C++/UnitScript | implemented | DUMMY marker. `custom_druid_thorns_rejuv_unitscript::OnDamage` → when Victim=Player + HasAura(901069) + has Thorns → 20% chance → CastSpell(Rejuv R15, triggered). |
