...
|---|----------|--------|--------|--------|---------|
| 1 | 900502 | Pet damage +50% | C++/UnitScript | implemented | DUMMY marker. `custom_hunter_pet_unitscript::OnDamage` → when Attacker=Pet + Owner HasAura(900502) → damage *= 1.5f. |
...
| 3 | 900504 | Pet AoE damage proc | C++/UnitScript | implemented | DUMMY marker. `custom_hunter_pet_aoe_unitscript::OnDamage` → when the pet hits + Owner HasAura(900504) → 15% chance → CastSpell(900505 Beast Cleave). |
