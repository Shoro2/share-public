...
|---|----------|--------|--------|--------|---------|
| 1 | 900666 | Energy regen +50% | DBC | nicht getestet | SPELL_AURA_MOD_POWER_REGEN_PERCENT (110), MiscValue=3 (Energy). BasePoints=50. |
| 2 | 900667 | Hemorrhage +50% damage | DBC | nicht getestet | ADD_PCT_MODIFIER (108) + SPELLMOD_DAMAGE (0). EffectSpellClassMaskA=0x2000000 targets Hemo. |
| 3 | 900668 | Hemorrhage +9 targets | C++ | nicht getestet | DUMMY Marker. C++ SpellScript auf Hemo (-48660): `AfterHit` → findet bis zu 9 Feinde im 8yd Radius → CastCustomSpell(900669, damage) auf jeden. Prüft `HasAura(900668)`. |
| H1 | 900669 | Helper: Deep Cut | DBC | nicht getestet | Instant Physical single-target damage. BasePoints via CastCustomSpell. |
