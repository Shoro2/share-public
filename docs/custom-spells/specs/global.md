...
| 2 | 901101 | Kill enemy → heal 5% total HP | C++ | implemented | Passive Proc aura: `PROC_FLAG_KILL` (0x1). `HandleProc`: `GetCaster()->CastCustomSpell(Heal-Helper, SPELLVALUE_BASE_POINT0, MaxHealth * 5 / 100, GetCaster(), triggered=true)`. Or: `GetCaster()->ModifyHealth(MaxHealth * 0.05)`. Simple on-kill heal. No ICD needed (kill events are naturally rate-limited). May need  Helper-Heal-Spell (e.g. 901105) for combat log visibility. |
