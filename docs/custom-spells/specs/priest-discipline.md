...
|---|----------|--------|--------|--------|---------|
| 1 | 900900 | Shields explode on breaking/fading | C++ | implemented | Power Word: Shield (48066) and other absorb shields. AuraScript `OnRemove`: If RemoveMode = EXPIRE (fade) or ENEMY_SPELL (break by damage) → CastSpell(Shield-Explosion-Helper, triggered=true) centered on the shield target. Explosion = Holy/Shadow AoE damage in a radius, Damage scales with the remaining/absorbed shield amount. Needs a helper AoE spell (e.g. 900903). Very cool thematically — Disc becomes AoE DPS via shields. |
| 2 | 900901 | Shields +50% | DBC | implemented | Passive aura: `SPELL_AURA_ADD_PCT_MODIFIER` +50% on PW:Shield (48066) absorb amount. SpellFamilyName=6, SpellFamilyFlags for PW:S. Increases the absorb value by 50%. Stacks with existing talents (Improved PW:S, Twin Disciplines, etc.). |
